...
class _EmployeeReportScreenState extends State<EmployeeReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final issues = currentUserId == null
        ? <IssueModel>[]
    ...
    final categories =
        <String>[
          'All',
          ...issues.map((issue) => issue.category).toSet().toList(),
        ]..sort(
          (a, b) => a == 'All'
              ? -1
              : b == 'All'
    ...
    final filteredIssues = issues.where((issue) {
      ...
      final matchesPriority =
          ...
          issue.priority.toLowerCase() == _selectedPriority.toLowerCase();
      final matchesStatus =
          _selectedStatus == 'All' ||
          issue.status.toLowerCase() == _selectedStatus.toLowerCase();
  ...
  Map<String, dynamic> _buildIssueData(IssueModel issue) {
    ...
    final severityMap = {
      'high': (const Color(0xFFFFEAEA), const Color(0xFFE15B5B)),
      'medium': (const Color(0xFFFFF2C9), const Color(0xFFC28200)),
      'low': (const Color(0xFFEAF7F1), const Color(0xFF2F8F5B)),
    };
    final status = issue.status.toLowerCase();
    final statusMap = {
      'pending': (const Color(0xFFF8F3D9), const Color(0xFFB58410)),
      'in progress': (const Color(0xFFEAF7F1), const Color(0xFF2F8F5B)),
      'resolved': (const Color(0xFFEAECEF), const Color(0xFF6B7280)),
      'reviewed': (const Color(0xFFEAECEF), const Color(0xFF6B7280)),
    };

    final statusStyle =
        statusMap[status] ?? (const Color(0xFFEAF7F1), const Color(0xFF2F8F5B));
    final severityStyle =
        severityMap[severity] ??
        (const Color(0xFFFFF2C9), const Color(0xFFC28200));
  ...
  Map<String, Color> _categoryColor(String category) {
    ...
    final map = {
      ...
      'safety': {
        'background': const Color(0xFFF8F1D8),
        'text': const Color(0xFFB77A00),
      },
      'structural': {
